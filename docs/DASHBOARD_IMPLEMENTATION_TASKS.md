...
| E.4 | **Hosted demo** or single-command `docker compose` for marketing | **Done** (local) — `docker compose up -d --build` + `scripts/verify-stack.sh`; public image publish / paid hosting remains out of repo |
